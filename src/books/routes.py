...
async def update_book(book_uid: int, update_book: updateBookModel, session: AsyncSession = Depends(get_session)) -> dict:
    updated_book = book_service.update_book(book_uid, update_book, session)
    if updated_book:
        return update_book
    else:
        raise HTTPException(
